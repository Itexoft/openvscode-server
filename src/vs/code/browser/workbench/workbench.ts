...
function normalizePreloadLinks(): void {
	...
	const qualify = () => {
		document.querySelectorAll<HTMLLinkElement>('link[rel="preload"]').forEach(link => {
			...
			const sameOrigin = (() => {
				if (parsedUrl) {
					return parsedUrl.origin === document.location.origin;
				}
...
class WorkspaceProvider implements IWorkspaceProvider {
	...
	static create(config: IWorkbenchConstructionOptions & { folderUri?: UriComponents; workspaceUri?: UriComponents }) {
		...
		if ((globalThis as any).__openvscodeEnableAllProposedApi === true && typeof payload === 'object' && payload && !('enableProposedApi' in payload)) {
			(payload as any).enableProposedApi = '*';
		}
...
(function () {
	...
	const originalConfig: IWorkbenchConstructionOptions & { folderUri?: UriComponents; workspaceUri?: UriComponents; callbackRoute: string; openvscode?: { verboseConsole?: boolean; webviewDebug?: boolean; enableProposedApiAll?: boolean; connectionToken?: string } } = JSON.parse(configElementAttribute);
	...
	if (openvscodeConfig.enableProposedApiAll) {
		(globalThis as any).__openvscodeEnableAllProposedApi = true;
	}
	if (openvscodeConfig.connectionToken) {
		(globalThis as any).__openvscodeConnectionToken = openvscodeConfig.connectionToken;
	}
